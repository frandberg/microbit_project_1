    ...
    if (game_stage == GAME_STAGE_PLAYING) {
        if (msg_kind == MSG_PLAYER_FINISH_TURN) {
            ...
            if (players_left_to_call != 0) {
                send_message(current_player, MSG_PLAYER_START_TURN, highest_bet.toString())
            }
...
function msg_recieved_player(sender: number, msg_kind: number, msg_contents: string) {
    if (game_stage == GAME_STAGE_FINDING_PLAYERS) {
        if (msg_kind == MSG_JOIN_GAME_PING){
            dealer_id = sender
...
let GAME_STAGE_FINDING_PLAYERS = 1
let GAME_STAGE_WAITING_FOR_GAME_TO_START = 2
let GAME_STAGE_PLAYING = 3
let ROLE_PLAYER = 1
let MSG_JOIN_GAME_PING = 0
let MSG_START_GAME = 1
let MSG_PLAYER_JOIN_CONFIRM = 2
let MSG_SEARCHING_FOR_PLAYERS = 3
let MSG_PLAYER_START_TURN = 4
let MSG_PLAYER_FINISH_TURN = 5
let highest_bet = 0
